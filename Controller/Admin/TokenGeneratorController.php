...
namespace Plugin\ApiToken\Controller\Admin;
...
class TokenGeneratorController extends AbstractController
{
     ...
     *
     * @Template("@ApiToken/admin/TokenGenerator/index.twig")
     */
    ...
    public function generate(Request $request, string $identifier): JsonResponse
    {
        ...
        try {
            ...
            log_info('ApiToken: Step1 validateAuthorizationRequest', ['redirect_uri' => $redirectUri]);
            ...
            log_info('ApiToken: Step2 completeAuthorizationRequest');
            ...
            log_info('ApiToken: Step3 code', ['code_exists' => $code !== null]);

            if (null === $code) {
                return $this->json(['error' => '認証コードの生成に失敗しました'], 500);
            }

            log_info('ApiToken: Step4 respondToAccessTokenRequest');
            ...
            log_info('ApiToken: 完了', ['has_access_token' => isset($tokens['access_token'])]);

            return $this->json($tokens);
        } catch (OAuthServerException $e) {
            log_error('ApiToken: OAuthServerException', [$e->getMessage(), $e->getHint() ?? '']);

            return $this->json(['error' => $e->getMessage()], $e->getHttpStatusCode());
        } catch (\Throwable $e) {
            log_error('ApiToken: Throwable', [$e->getMessage(), $e->getFile().':'.$e->getLine()]);
